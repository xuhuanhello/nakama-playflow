...
| RPC | Payload | Result |
| --- | --- | --- |
| `fleet_assignment_get_v1` | `{}` or `{"allocation_id":"..."}` | Own current allocation; signed admission fields only when assigned/active and eligible |
| `fleet_resume_v1` | `{"allocation_id":"..."}` | Same owned seat, fresh nonce and short resume ticket within reconnect policy |
